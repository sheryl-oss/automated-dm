"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var repository_1 = require("../core/repository");
var errors_1 = require("../errors");
var lodash_1 = require("lodash");
var Chance = require("chance");
var UserRepository = /** @class */ (function (_super) {
    __extends(UserRepository, _super);
    function UserRepository() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    UserRepository.prototype.info = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.client.request.send({
                            url: "/api/v1/users/" + id + "/info/"
                        })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body.user];
                }
            });
        });
    };
    UserRepository.prototype.arlinkDownloadInfo = function () {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.client.request.send({
                            url: "/api/v1/users/arlink_download_info/",
                            qs: {
                                version_override: '2.0.2'
                            }
                        })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body.user];
                }
            });
        });
    };
    UserRepository.prototype.search = function (username) {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.client.request.send({
                            url: "/api/v1/users/search/",
                            qs: {
                                timezone_offset: this.client.state.timezoneOffset,
                                q: username,
                                count: 30
                            }
                        })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body];
                }
            });
        });
    };
    UserRepository.prototype.searchExact = function (username) {
        return __awaiter(this, void 0, void 0, function () {
            var result, users, account;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        username = username.toLowerCase();
                        return [4 /*yield*/, this.search(username)];
                    case 1:
                        result = _a.sent();
                        users = result.users;
                        account = users.find(function (user) { return user.username === username; });
                        if (!account) {
                            throw new errors_1.IgExactUserNotFoundError();
                        }
                        return [2 /*return*/, account];
                }
            });
        });
    };
    UserRepository.prototype.accountDetails = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        id = id || this.client.state.cookieUserId;
                        return [4 /*yield*/, this.client.request.send({
                                url: "/api/v1/users/" + id + "/account_details/"
                            })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body];
                }
            });
        });
    };
    UserRepository.prototype.formerUsernames = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        id = id || this.client.state.cookieUserId;
                        return [4 /*yield*/, this.client.request.send({
                                url: "/api/v1/users/" + id + "/former_usernames/"
                            })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body];
                }
            });
        });
    };
    UserRepository.prototype.sharedFollowerAccounts = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.client.request.send({
                            url: "/api/v1/users/" + id + "/shared_follower_accounts/"
                        })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body];
                }
            });
        });
    };
    UserRepository.prototype.flagUser = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.client.request.send({
                            url: "/api/v1/users/" + id + "/flag_user/",
                            method: 'POST',
                            form: {
                                _csrftoken: this.client.state.cookieCsrfToken,
                                _uid: this.client.state.cookieUserId,
                                _uuid: this.client.state.uuid,
                                reason_id: 1,
                                user_id: id,
                                source_name: 'profile',
                                is_spam: true
                            }
                        })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body];
                }
            });
        });
    };
    UserRepository.prototype.getIdByUsername = function (username) {
        return __awaiter(this, void 0, void 0, function () {
            var user;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.searchExact(username)];
                    case 1:
                        user = _a.sent();
                        return [2 /*return*/, user.pk];
                }
            });
        });
    };
    UserRepository.prototype.lookup = function (options) {
        return __awaiter(this, void 0, void 0, function () {
            var body;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = lodash_1.defaults(options, {
                            waterfallId: new Chance().guid({ version: 4 }),
                            directlySignIn: true,
                            countryCodes: [{ country_code: '1', source: ['default'] }]
                        });
                        return [4 /*yield*/, this.client.request.send({
                                url: '/api/v1/users/lookup/',
                                method: 'POST',
                                form: this.client.request.sign({
                                    country_codes: JSON.stringify(options.countryCodes),
                                    _csrftoken: this.client.state.cookieCsrfToken,
                                    q: options.query,
                                    guid: this.client.state.uuid,
                                    device_id: this.client.state.deviceId,
                                    waterfall_id: options.waterfallId,
                                    directly_sign_in: options.directlySignIn.toString()
                                })
                            })];
                    case 1:
                        body = (_a.sent()).body;
                        return [2 /*return*/, body];
                }
            });
        });
    };
    return UserRepository;
}(repository_1.Repository));
exports.UserRepository = UserRepository;
